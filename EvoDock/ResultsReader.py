...
MODE_SCORES = "SCORES" # checks scores and time
MODE_NUMBER = "NUMBER" # checks only number of individuals in history
mode = MODE_SCORES
...
for directory in dirs:
    ...
    if mode == MODE_SCORES:
        ...
        summary.append(lines[0].split(':')[1].strip())
...
if mode == MODE_SCORES:
    ...
    for time in times:
        try:
            ...
            print("WARNING: Value error in time calculation!")
        # write to worksheet
    ...
    col = 2
    for entry in summary:
        worksheet.write(2, col, float(entry))
        col += 1
